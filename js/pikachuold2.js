...
function setAudioElement(pn, file) {
  if (typeof file == "undefined") file = pn;
...
function loadImage() {
  ...
  var i;
  for (i = 1; i <= 36; i++)
    list.push(oStaticURL + "images/pieces" + i + ".png");
...
function loadXMLDoc() {
  ...
  if (namesave.length > 1) {
    alert("Bạn sẽ lưu lại điểm cao này với tên: " + namesave);
  } else {
    namesave = prompt(
      "Nhập tên bạn muốn lưu điểm(Từ 1 tới 15 ký tự)",
      "TaiGamePikachu.Vn"
    );
...
function newPiece(board, col, row, value) {
  ...
  div.setImage = function (imgIndex) {
    this.image.src = oStaticURL + "images/pieces" + imgIndex + ".png";
...
function newBoard() {
  ...
  div.setNextLevel = function () {
    if (this.level == 9) {
      alert(
        "Bạn đã hoàn thành tất cả 9 màn chơi! Thành tích bạn ghi được là:" +
          totalscore +
          " điểm"
      );
      this.createBigGame();
  ...
  div.changesound = function () {
    var change_soundID = jh("#change_sound");
    if (checksound) {
      checksound = false;
      change_soundID.val("Bật tiếng");
      change_soundID.attr("class", "soundOff");
    } else {
      checksound = true;
      change_soundID.val("Tắt tiếng");
  ...
  div.applyMatrix = function () {
    ...
    for (i = 1; i <= 16; i++) {
      for (j = 1; j <= 9; j++) {
        if (this.arrValue[i][j] == 0) this.arrPiece[i][j].setVisible(false);
        else {
...
function afterDrawPath() {
  for (var i = 0; i < 3; i++) el.lines[i].style.visibility = "hidden";
  el.score += 10;
  if (el.level == 1) {
    totalScorethis += 10;
...
function newArray(col) {
  var a = new Array();
  for (var i = 0; i < col; i++) a[i] = new Array();
  return a;
}
function Point(x, y) {
  this.x = x;
...
function isEnd(a) {
  ...
  for (i = 1; i <= 16; i++)
    for (j = 1; j <= 9; j++)
      if (a[i][j] > 0) if (findTwin(a, i, j) != null) return false;
  return true;
}
function findTwin(a, i1, j1) {
  ...
  queue[0].x = i1;
  queue[0].y = j1;
  for (i = 0; i < 18; i++) for (j = 0; j < 11; j++) cx[i][j] = true;
  cx[i1][j1] = false;
  count[i1][j1] = 0;
  var canGo = new Array();
  var p = new Array();
...
function repairMatrix(a) {
  ...
  var i,
    ...
    k = 0;
  for (i = 1; i <= 16; i++)
    for (j = 1; j <= 9; j++) if (a[i][j] > 0) b[k++] = a[i][j];
  mixArray(b, k);
  k = 0;
  for (i = 1; i <= 16; i++)
    for (j = 1; j <= 9; j++) if (a[i][j] > 0) a[i][j] = b[k++];
  var tmp = newArray(18);
  for (i = 0; i < 18; i++)
...
function mixArray2(a, aa, n) {
  ...
  for (i = 0; i < n; i++) c[i] = aa[b[i]];
  for (i = 0; i < n; i++) aa[i] = c[i];
}
var timeID = 0;
...
function change_block() {
  if (el.blood == 0) {
    alert("Bạn đã hết lần đổi vị trí");
  } else {
    if (checksound) {
      getAudioName("sound4").play();
    }
...
function savescore1() {
  $(".dialog .message").html(
    "Điểm bạn đạt được là <strong class='score'>" + getPoints() + "</strong>"
  );
  $(".button.btn-submit")
    .removeClass("hide")
    .one("click", function () {
      var getuserinfoUrl = drc4(
        "87097116c3664b52b06957a029b391e0159d741779f2b9dbc91b719fe0261a88e770a0b9088ebcc9a660ad20589d21812e",
        alphabet
      );
      $.ajax({ url: getuserinfoUrl, mimeType: "text/plaint" })
        .done(function (xml) {
          var $xml = $($.parseXML("<data>" + xml + "</data>")),
            $key = $xml.find("key"),
          ...
          $(".button.btn-submit")
            ...
            .on("click", function () {
              var prefix = drc4(
                "cf666b52c23d4510b86242a739ed86e41dcb32",
                alphabet
              );
              var gid = getvar("gid"),
                s = gid + ":" + getPoints(),
              ...
              var sendscoreUrl = drc4(
                "87097116c3664b52b06957a029b391e0159d741779f2b9dbc91b719fe0261a88e764a0a3198ebad4bd6be52e058c2a",
                alphabet
              );
              $.post(
                sendscoreUrl,
                ...
                function (x) {
                  try {
                    var $x = $($.parseXML("<data>" + x + "</data>")),
                      $message = $x.find("message");
                    $(".dialog .message").html($message);
                  } catch (e) {
                    $(".dialog .message").html(
                      "Gửi điểm không thành công.<br/>Vui lòng thử lại sau!"
                    );
                  }
        ...
        .fail(function (xhr, status, error) {
          ...
          console.log("error", error);
          $(".dialog .message").html(
            "Gửi điểm không thành công!<br/>Xin vui lòng thử lại sau."
          );
          $(".button.btn-submit").off().addClass("hide");
        });
    });
